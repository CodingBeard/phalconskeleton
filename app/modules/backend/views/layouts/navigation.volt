...
<nav role="navigation">
  <ul class="hide-on-med-and-down">
    ...
    {% for navlink in _backendnav.getNavlinks('level = 1') %}
      {% if navlink.inUrl(_SERVER['REQUEST_URI']) and navlink.children is iterable %}
        <li>
          <a data-group="{{ navlink.parent.id }}" data-parent="{{ navlink.id }}"
             href="{{ url(navlink.link) }}">{{ navlink.label }}</a>
        </li>
  ...
  {% cache "backend_navbar" ~ _backendnav.navlinks.getFirst().id %}
  <div class="container">
          ...
          {% if navlink.children is iterable %}
            <li class="no-padding">
              <ul class="collapsible">
                <li class="bold">
                  <a data-id="{{ navlink.id }}" class="collapsible-header"
                     data-activates="collapse1">{{ navlink.label }}</a>
